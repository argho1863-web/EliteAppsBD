...
import { Eye, EyeOff, Zap, Mail, Lock, LogIn, AlertCircle } from 'lucide-react';
...
function SignInContent() {
  ...
  useEffect(() => {
    ...
    if (error) toast.error(error === 'CredentialsSignin' ? 'Invalid email or password' : 'Authentication failed');
  }, [searchParams]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res?.error) toast.error(res.error === 'CredentialsSignin' ? 'Invalid email or password' : res.error);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-6 relative overflow-hidden bg-brand-navy">
      ...
      <div className="absolute top-0 left-0 w-full h-full overflow-hidden pointer-events-none">
        <div className="absolute -top-[10%] -left-[10%] w-[40%] h-[40%] bg-brand-gold/10 blur-[120px] rounded-full animate-pulse" />
        <div className="absolute -bottom-[10%] -right-[10%] w-[40%] h-[40%] bg-brand-electric/10 blur-[120px] rounded-full animate-pulse" style={{ animationDelay: '2s' }} />
      </div>
      ...
      <div className="relative z-10 w-full max-w-md animate-fade-in">
        <div className="text-center mb-10">
          ...
          <h1 className="mt-8 text-responsive-h2 text-white">Welcome back</h1>
          <p className="text-sm font-medium text-white/30 mt-2 uppercase tracking-widest">Sign in to your account</p>
        </div>
        ...
        <div className="glass rounded-[2rem] p-8 sm:p-10 border border-white/5 shadow-2xl shadow-black/40">
          <button onClick={handleGoogle} disabled={googleLoading}
            className="w-full flex items-center justify-center gap-4 bg-white/5 hover:bg-white/10 border border-white/10 hover:border-white/20 rounded-2xl py-4 text-sm font-black uppercase tracking-widest text-white transition-all mb-8 disabled:opacity-50 group active:scale-95 shadow-lg">
          ...
          <div className="flex items-center gap-4 mb-8">
            ...
            <span className="text-[10px] text-white/20 uppercase font-black tracking-[0.2em]">or use email</span>
            <div className="flex-1 h-px bg-white/5" />
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="name@email.com" className="input-dark h-14 pl-12 pr-6 text-base font-medium" autoComplete="email" required />
              </div>
            </div>
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <input type={showPass ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)} placeholder="••••••••" className="input-dark h-14 pl-12 pr-12 text-base font-medium" autoComplete="current-password" required />
            ...
            <button type="submit" disabled={loading} className="btn-gold w-full py-4 rounded-2xl font-black uppercase tracking-widest text-xs flex items-center justify-center gap-3 mt-4 disabled:opacity-50 shadow-gold active:scale-95 transition-all">
              {loading ? <div className="w-5 h-5 border-2 border-brand-navy/30 border-t-brand-navy rounded-full animate-spin" /> : <><LogIn size={18} /> Sign In Now</>}
            </button>
          </form>
        </div>
        ...
        <p className="text-center text-sm font-medium text-white/30 mt-8">
          New to EliteApps?{' '}
          <Link href="/auth/signup" className="text-brand-gold hover:text-white transition-colors underline underline-offset-4 decoration-brand-gold/30">Create an account</Link>
        </p>
      </div>
    </div>
  );
