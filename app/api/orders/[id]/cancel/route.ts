...
import { sendOrderCancellationToCustomer } from '@/lib/email';
...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    await db.updateOne('orders', { _id: id }, { $set: { status: 'cancelled', updatedAt: new Date().toISOString() } });
    await sendOrderCancellationToCustomer(order.userEmail, order.userName, id);
    return new NextResponse(`<!DOCTYPE html><html><body style="font-family:'Segoe UI',sans-serif;text-align:center;padding:60px;background:#0a0a14;color:#e2e8f0;">
      <div style="max-width:400px;margin:0 auto;background:#12121e;padding:40px;border-radius:16px;border:2px solid #ef4444;">
        <div style="font-size:56px;margin-bottom:12px;">❌</div>
        <h2 style="color:#ef4444;margin:0 0 10px;">Order Cancelled</h2>
        <p style="color:#94a3b8;">Order #${id.slice(-8).toUpperCase()} cancelled.</p>
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    await db.updateOne('orders', { _id: id }, { $set: { status: 'cancelled', updatedAt: new Date().toISOString() } });
    await sendOrderCancellationToCustomer(order.userEmail, order.userName, id);
    return NextResponse.json({ message: 'Order cancelled and customer notified' });
  } catch (error: any) { return NextResponse.json({ error: error.message }, { status: 500 }); }
}
